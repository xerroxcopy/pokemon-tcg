  ...
  ~ mutate(
    ...
    game_gen = 
      case_when(
        ...
        release_date > game_gen_df |> filter(game_gen == .x) |> extract("game_release_date") |> pull()
        ~ .x,
        TRUE ~ NA_integer_
      ),
    # game_gen_ja = 
...
series_gen_df2$cards_total |> na.omit() |> sum()
```
...
# random_draw_df1 |> str()
# random_draw_df1 |> View()
series_gen_df3$cards_total |> na.omit() |> sum()
```
### unnest now

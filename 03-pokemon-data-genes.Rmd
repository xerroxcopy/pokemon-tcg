...
[leading zeros](https://stackoverflow.com/questions/8266915/format-number-as-fixed-width-with-leading-zeros)

```{r fig.width = 5, fig.height = 5}
# image_file_names <- list.files("./input/bulbapedia_image_temp/", pattern = "png")
# new_names <- image_file_names |> str_remove("70px-") |> str_remove("(?<=[:digit:]{3}).+(?=\\.)")
# file.rename(
#   paste0("./input/bulbapedia_image_temp/",image_file_names),
#   to = paste0("./input/bulbapedia_image/", new_names)
# )
df_names |> 
 
  ggplot(
    aes(x = pokedex_id %/% 30, y = pokedex_id %% 30, image = path)) +
  geom_image(size = .04) +
  theme_pokemon +
  theme(
    aspect.ratio = 1

...
|`gen`|generation of the series, from 1-8. This is based on V3.25 spreadsheet and may not match from Bulbapedia's classification (e.g., `Legendary Collection` classified as gen 1 in spreadsheet, gen 2 in Bulbapedia).|int|
...
Now that the series names are Bulbanized, merge these datasets to retrieve `series_type` and `release_date` and  `gen`:

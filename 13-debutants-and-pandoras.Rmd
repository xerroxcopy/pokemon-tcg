...
p_sim_emp_debut_cumsum <-
  ...
  theme_pokemon
ggsave("./output/p_total_debutant.png", width = 166/4, height = 100/4, unit = "mm", dpi = 1200)
ggsave("./output/p_total_debutant.pdf", width = 166/4, height = 100/4, unit = "mm", dpi = 1200) # pdf renders violin better than svg
```
### residual plot
...
df_debut_sim_emp <- df_debut_sim |> 
  ...
  left_join(df_series3, by = c("release_date"))
...
df_debut_mean <- df_debut_sim_emp |> 
      group_by(release_date) |> 
      summarise(mean_residual = mean(residual), release_gen) |>
      distinct()

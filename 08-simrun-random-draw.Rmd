...
この日付以降。
```{r}
series_gen_df4_actual_release_gen <-
# df_view <-
  series_gen_df2 |> 
  mutate(
    actual_release_gen = case_when(
      release_date < series_gen_actual_df2$release_date[2] ~ 1,
      release_date < series_gen_actual_df2$release_date[3] ~ 2,
      release_date < series_gen_actual_df2$release_date[4] ~ 3,
      release_date < series_gen_actual_df2$release_date[5] ~ 4,
      release_date < series_gen_actual_df2$release_date[6] ~ 5,
      release_date < series_gen_actual_df2$release_date[7] ~ 6,
      release_date < series_gen_actual_df2$release_date[8] ~ 7,
      TRUE ~ 8,
    )
  ) |> 
  left_join(
    df_game_gen |> select(game_gen, pokemon_cumulative_n), 
    by = c("actual_release_gen" = "game_gen")) |> 
  filter(!is.na(pokemon_data_count))
...
for (simulation_run in 1:simulation_runs) {
  single_sim_result_df <- series_gen_df4_actual_release_gen |>
  mutate(

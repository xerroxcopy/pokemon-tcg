...
pokemon_type_ranking <- cards_df_typo |>
  filter(!card_type %in% types_non_pokemon) |> 
  group_by(card_type) |> 
  summarise(n = n()) |> 
  arrange(desc(n))
...
df_type_colour <- tribble(
  ~ card_type, ~ rgb, ~ colour,
      "Water", "#80DDFF", "lightblue",
      "Grass", "#80FF82", "green",
      "Psychic", "#361C63", "purple",
      "Colorless", "#D1D1D1", "lightgrey",
      "Fighting", "#8F441E", "orange",
      "Fire", "#FF0F23", "red",
      "Lightning", "#F0C800", "yellow",
      "Darkness", "#1D1D1B", "darkgrey",
      "Metal", "#463F43", "silver",
      "Dragon", "#ABAD00", "gold",
      "Fairy", "#FF4DAF", "pink"
)
...
cards_df_date <- cards_df_typo|> 
  mutate(
    card_type2 = case_when(
      card_type %in% mixed_types ~ "mixed",
      TRUE ~ card_type),
    is_pokemon = !card_type %in% types_non_pokemon & card_name != "Buried Fossil"
